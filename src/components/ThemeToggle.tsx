...
export default function ThemeToggle() {
  // null = not yet mounted (avoid SSR/client mismatch)
  ...
  // Don't render until we know the real theme — avoids icon flash
  if (isDark === null) {
    ...
    <>
      ...
      {showWarning && createPortal(
        <div
          ...
          style={{ zIndex: 99999, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(6px)' }}
          onClick={e => { if (e.target === e.currentTarget) setShowWarning(false) }}
        >
          <div
            className="bg-[#1a1a1a] border border-[#f0ede8]/10 rounded-2xl p-8 max-w-[320px] w-[90%] shadow-[0_32px_80px_rgba(0,0,0,0.7)]"
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => { setShowWarning(false); applyTheme(false) }}
                className="flex-1 py-2.5 rounded-lg bg-[#4a9ebb] border-none
